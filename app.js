// === THINKING ORBS CANVAS ENGINE (Jakub Antalik) ===
/**
 * Official Thinking Orbs (vanilla JS engine) by Jakub Antalik
 * Wrapped for ethos.init terminal app.
 */
(function(window) {
...
function Gt({
  ...
  window.ThinkingOrbsEngine = {
    MODE_DRAWS: Nt,
    STATE_TO_MODE: Tt,
    resolvePreset: Ft,
    createOrb: function(canvas, options) {
      options = options || {};
      let state = options.state || 'working';
      let size = options.size || 64;
      let theme = options.theme || 'dark'; // default dark ink for ethos
      ...
      function drawFrame(t) {
        if (!ctx) return;
        const preset = Ft(state, size);
        const drawFn = Nt[preset.mode];
        const effectiveSpeed = preset.speed * speed;
        
        ctx.setTransform(dpr, 0, 0, dpr, 0, 0);
        ctx.clearRect(0, 0, size, size);
        
        const isDark = theme === 'dark' || (theme === 'auto' && (document.documentElement.classList.contains('dark') || document.documentElement.dataset.theme === 'dark' || (window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)').matches)));
        drawFn(ctx, size, t * effectiveSpeed, isDark, preset.opts);
      }
