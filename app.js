...
function init() {
  ...
  initTabs(); initButtons(); render(); startClock(); startFlowerAnimation(); startSakuraAmbientAnimation();
...
function toggleEthos(rIdx, eId) {
  ...
  if (e.done) {
    ...
    if (window.triggerSakuraCompletionBurst) {
      window.triggerSakuraCompletionBurst();
    }
    var all = getAllEthe();
...
// === AMBIENT SAKURA BACKGROUND ENGINE ===
let sakuraParticles = [];
let sakuraAmbientStarted = false;
function startSakuraAmbientAnimation() {
  if (sakuraAmbientStarted) return;
  const canvas = document.getElementById('sakura-ambient');
  if (!canvas) return;
  sakuraAmbientStarted = true;
  const ctx = canvas.getContext('2d');
  ...
  // Generate left and right organic vertical branches
  // We represent the branches as nodes that scale dynamically on draw
  const leftBranch = generateProceduralBranch(false);
  const rightBranch = generateProceduralBranch(true);

  // Pre-allocate particle pool to maintain a constant memory footprint (0% overhead)
  const maxParticles = 120;
  sakuraParticles = Array.from({ length: maxParticles }, () => ({ active: false }));

  function generateProceduralBranch(isRightSide) {
    const nodes = [];
    let yRatio = 1.0;
    let xRatio = 0.5; // Start in middle of margin [0..1]
    let seed = isRightSide ? 888 : 111;
    ...
    while (yRatio > 0.05) {
      yRatio -= 0.015 + prng() * 0.012;
      xRatio += (prng() - 0.5) * 0.06;
      if (xRatio < 0.1) xRatio = 0.1;
      ...
      const blossoms = [];
      // High blossom density on upper branches
      const blossomProb = yRatio < 0.5 ? 0.85 : 0.6;
      if (prng() < blossomProb) {
        const numB = Math.floor(prng() * 4) + 1; // 1 to 4 blossoms per node
        for (let b = 0; b < numB; b++) {
          blossoms.push({
            dxRatio: (prng() - 0.5) * 0.12,
            dyRatio: (prng() - 0.5) * 0.025,
            char: ['*', 'x', '@', '+', '%', '.', 'o'][Math.floor(prng() * 7)],
            colorType: prng() < 0.55 ? 'light' : (prng() < 0.8 ? 'dark' : 'accent'),
      ...
      // Procedural side-twigs spawning
      if (prng() < 0.15 && yRatio < 0.85) {
        let twigY = yRatio;
        let twigX = xRatio;
        let twigLength = 4 + Math.floor(prng() * 6);
        let twigDir = prng() < 0.5 ? -1 : 1;

        for (let t = 0; t < twigLength; t++) {
          twigY -= 0.012;
          twigX += twigDir * 0.035;
          if (twigX < 0.05) twigX = 0.05;
          if (twigX > 0.95) twigX = 0.95;

          let twigChar = twigDir > 0 ? '╱' : '╲';
          const twigBlossoms = [];
          if (prng() < 0.8) {
            const numB = Math.floor(prng() * 3) + 1;
            for (let b = 0; b < numB; b++) {
              twigBlossoms.push({
                dxRatio: (prng() - 0.5) * 0.08,
                dyRatio: (prng() - 0.5) * 0.02,
                char: ['*', 'x', '+', '.', 'o'][Math.floor(prng() * 5)],
                colorType: prng() < 0.65 ? 'light' : 'dark',
                phase: prng() * Math.PI * 2
              });
            }
          }
  ...
  function spawnSakuraParticle(xRatio, yRatio, char = null, colorType = null, speedMult = 1.0) {
    const p = sakuraParticles.find(x => !x.active);
    if (!p) return;

    p.active = true;
    p.xRatio = xRatio !== null ? xRatio : Math.random();
    p.yRatio = yRatio !== null ? yRatio : -0.02;
    p.vxRatio = (Math.random() - 0.5) * 0.01 * speedMult;
    p.vyRatio = (0.004 + Math.random() * 0.008) * speedMult;
    p.char = char || ['*', 'x', '+', '.', 'o'][Math.floor(Math.random() * 5)];
    p.colorType = colorType || (Math.random() < 0.6 ? 'light' : 'dark');
    p.rotation = Math.random() * Math.PI * 2;
    p.rotationSpeed = (Math.random() - 0.5) * 0.05 * speedMult;
    p.alpha = 0.4 + Math.random() * 0.6;
    p.scale = 0.7 + Math.random() * 0.6;
    p.phase = Math.random() * Math.PI * 2;
  }

  // Completion burst trigger registered globally
  window.triggerSakuraCompletionBurst = function() {
    const count = 18 + Math.floor(Math.random() * 8);
    for (let i = 0; i < count; i++) {
      // Spawn scattered from left or right top/middle quadrant
      let side = Math.random() < 0.5;
      let xRatio = side ? (0.86 + Math.random() * 0.12) : (Math.random() * 0.12);
      let yRatio = 0.1 + Math.random() * 0.4;
      spawnSakuraParticle(xRatio, yRatio, null, 'accent', 1.6);
    }
  };

  // Main Draw Tick
  function tick() {
    // If canvas or tab is hidden, skip drawing to save power
    if (document.hidden) {
    ...
    const marginWidth = width * 0.12;

    ctx.clearRect(0, 0, width, height);

    const now = Date.now();
    const isFocusActive = focusSession && focusSession.active && focusSession.type === 'focus';
    const isOracleActive = !!window.oracleStreamingActive;

    // Wind sway solver: compound sine waves anchored to the bottom margins
    let tWind = now / 1400;
    let maxSwayAngle = 0.03;
    if (isFocusActive) {
      tWind = now / 700;
      maxSwayAngle = 0.065;
    ...
    const sway = Math.sin(tWind) * maxSwayAngle + Math.sin(tWind * 1.5 + 0.8) * (maxSwayAngle * 0.5);

    // Dynamic color palette loader from CSS custom properties
    const computedStyles = getComputedStyle(document.documentElement);
    const accentColor = computedStyles.getPropertyValue('--accent').trim() || '#00ffcc';
    const textDim = computedStyles.getPropertyValue('--text-dim').trim() || '#a0aec0';
    const textFaint = computedStyles.getPropertyValue('--text-faint').trim() || '#4a5568';
    const redColor = computedStyles.getPropertyValue('--red').trim() || '#ff0055';

    // Pink / neon variants of the accent color for beautiful cherry blossoms
    const lightPink = 'hsl(340, 100%, 82%)';
    const darkPink = 'hsl(335, 95%, 68%)';
    const oracleGreen = 'hsl(140, 100%, 70%)'; // glowing green when oracle streams

    function getBlossomColor(colorType) {
      if (isOracleActive) return oracleGreen;
      if (colorType === 'accent') return accentColor;
      if (colorType === 'light') return lightPink;
      return darkPink;
    }

    // Canvas fonts styling
    ctx.font = '11px monospace';
    ctx.textBaseline = 'middle';
    ctx.textAlign = 'center';

    // Draw a procedural branch
    function drawBranch(nodes, isRightSide) {
      nodes.forEach(node => {
        // Calculate raw coords
        let nx = isRightSide ? (width - marginWidth + node.xRatio * marginWidth) : (node.xRatio * marginWidth);
        let ny = height * node.yRatio;

        // Apply sway anchored at the bottom (yRatio = 1.0)
        let swayOffset = (height - ny) * sway;
        let finalX = nx + (isRightSide ? -swayOffset : swayOffset);

        // Render Branch character
        ctx.fillStyle = textFaint;
        ctx.fillText(node.char, finalX, ny);

        // Render attached blossoms
        node.blossoms.forEach(b => {
          let bx = finalX + b.dxRatio * marginWidth;
          let by = ny + b.dyRatio * height;
          
          // Micro-fluttering effect on individual blossoms
          let flutter = Math.sin(now / 200 + b.phase) * (isFocusActive ? 1.5 : 0.6);
          
          ctx.fillStyle = getBlossomColor(b.colorType);
          
          // Make blossoms pulse slightly when Oracle is streaming
          if (isOracleActive) {
            ctx.shadowColor = oracleGreen;
            ctx.shadowBlur = 4 + Math.sin(now / 100) * 3;
          } else {
            ctx.shadowBlur = 0;
          }

          ctx.fillText(b.char, bx + flutter, by);
        });
      });
      ctx.shadowBlur = 0; // Reset shadow
    }

    // Render left & right branches
    drawBranch(leftBranch, false);
    drawBranch(rightBranch, true);

    // Particle pool spawner logic
    let ambientSpawnProb = isFocusActive ? 0.12 : 0.025;
    if (Math.random() < ambientSpawnProb) {
      spawnSakuraParticle(Math.random(), -0.03, null, null, isFocusActive ? 2.0 : 1.0);
    }

    // Branch-shedding particle spawner (organically drops petals from tree nodes)
    let sheddingProb = isFocusActive ? 0.06 : 0.015;
    if (Math.random() < sheddingProb) {
      const activeBranch = Math.random() < 0.5 ? leftBranch : rightBranch;
      if (activeBranch.length > 0) {
        const node = activeBranch[Math.floor(Math.random() * activeBranch.length)];
        let isRight = activeBranch === rightBranch;
        let nx = isRight ? (1.0 - 0.12 + 0.12 * node.xRatio) : (0.12 * node.xRatio);
        spawnSakuraParticle(nx, node.yRatio, node.blossoms[0]?.char, node.blossoms[0]?.colorType, isFocusActive ? 1.8 : 1.0);
      }
    }
    ...
    sakuraParticles.forEach(p => {
      ...
      // Update positions
      let speedMult = isFocusActive ? 2.2 : 1.0;
      p.yRatio += p.vyRatio * speedMult;
      
      // Floating wave offset in horizontal velocity
      let wave = Math.sin(now / 600 + p.phase) * 0.0015 * (isFocusActive ? 2.0 : 1.0);
      p.xRatio += (p.vxRatio + wave);
      ...
      if (p.yRatio > 1.02 || p.xRatio < -0.02 || p.xRatio > 1.02) {
        p.active = false;
        return;
      }
      ...
      ctx.fillStyle = getBlossomColor(p.colorType);
